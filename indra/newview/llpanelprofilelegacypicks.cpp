/** 
 * @file llpanelprofilelegacypicks.cpp
 * @brief LLPanelProfileLegacyPicks and related class implementations
 *
...
static LLPanelInjector<LLPanelProfileLegacyPicks> t_panel_picks("panel_picks");
...
public:
	...
	bool handle(const LLSD& params, const LLSD& query_map, LLMediaCtrl* web) override
	{
		...
		{
			LLNotificationsUtil::add("NoPicks", LLSD(), LLSD(), 
				std::string("SwitchToStandardSkinAndQuit"));
			return true;
		}
	...
	void createPick()
	{
		...
		LLPanelProfileLegacyPicks* picks = picks_floater->findChild<LLPanelProfileLegacyPicks>("panel_picks");
		if (picks)
		{
			picks->createNewPick();
		}
...
//-----------------------------------------------------------------------------
// LLPanelProfileLegacyPicks
//-----------------------------------------------------------------------------
LLPanelProfileLegacyPicks::LLPanelProfileLegacyPicks()
...
LLPanelProfileLegacyPicks::~LLPanelProfileLegacyPicks()
{
	if(LLPanelProfileLegacyTab::getAvatarId().notNull())
	{
		LLAvatarPropertiesProcessor::getInstance()->removeObserver(
			LLPanelProfileLegacyTab::getAvatarId(),this);
	}
...
void* LLPanelProfileLegacyPicks::create(void* data /* = NULL */)
{
	return new LLPanelProfileLegacyPicks();
}

void LLPanelProfileLegacyPicks::updateData()
{
...
void LLPanelProfileLegacyPicks::processProperties(void* data, EAvatarProcessorType type)
{
	if(APT_PICKS == type)
	{
		...
		if(avatar_picks && getAvatarId() == avatar_picks->target_id)
		{
			...
			for(; avatar_picks->picks_list.end() != it; ++it)
			{
				...
				picture->childSetAction("info_chevron", boost::bind(&LLPanelProfileLegacyPicks::onClickInfo, this));
				...
				picture->setDoubleClickCallback(boost::bind(&LLPanelProfileLegacyPicks::onDoubleClickPickItem, this, _1));
				picture->setRightMouseUpCallback(boost::bind(&LLPanelProfileLegacyPicks::onRightMouseUpItem, this, _1, _2, _3, _4));
				picture->setMouseUpCallback(boost::bind(&LLPanelProfileLegacyPicks::updateButtons, this));
			}
	...
	else if((APT_CLASSIFIEDS == type) || (APT_CLASSIFIED_INFO == type))
	{
		...
		if(c_info && getAvatarId() == c_info->target_id)
		{
			...
			for(; c_info->classifieds_list.end() != it; ++it)
			{
				...
				c_item->childSetAction("info_chevron", boost::bind(&LLPanelProfileLegacyPicks::onClickInfo, this));
				...
				c_item->setDoubleClickCallback(boost::bind(&LLPanelProfileLegacyPicks::onDoubleClickClassifiedItem, this, _1));
				c_item->setRightMouseUpCallback(boost::bind(&LLPanelProfileLegacyPicks::onRightMouseUpItem, this, _1, _2, _3, _4));
				c_item->setMouseUpCallback(boost::bind(&LLPanelProfileLegacyPicks::updateButtons, this));
			}
...
LLPickItem* LLPanelProfileLegacyPicks::getSelectedPickItem() const
{
...
LLClassifiedItem* LLPanelProfileLegacyPicks::getSelectedClassifiedItem() const
{
...
BOOL LLPanelProfileLegacyPicks::postBuild()
{
	...
	mPicksList->setCommitCallback(boost::bind(&LLPanelProfileLegacyPicks::onListCommit, this, mPicksList));
	mClassifiedsList->setCommitCallback(boost::bind(&LLPanelProfileLegacyPicks::onListCommit, this, mClassifiedsList));
	...
	childSetAction(XML_BTN_NEW, boost::bind(&LLPanelProfileLegacyPicks::onClickPlusBtn, this));
	childSetAction(XML_BTN_DELETE, boost::bind(&LLPanelProfileLegacyPicks::onClickDelete, this));
	childSetAction(XML_BTN_TELEPORT, boost::bind(&LLPanelProfileLegacyPicks::onClickTeleport, this));
	childSetAction(XML_BTN_SHOW_ON_MAP, boost::bind(&LLPanelProfileLegacyPicks::onClickMap, this));
	childSetAction(XML_BTN_INFO, boost::bind(&LLPanelProfileLegacyPicks::onClickInfo, this));

	mPicksAccTab = getChild<LLAccordionCtrlTab>("tab_picks");
	mPicksAccTab->setDropDownStateChangedCallback(boost::bind(&LLPanelProfileLegacyPicks::onAccordionStateChanged, this, mPicksAccTab));
	mPicksAccTab->setDisplayChildren(true);

	mClassifiedsAccTab = getChild<LLAccordionCtrlTab>("tab_classifieds");
	mClassifiedsAccTab->setDropDownStateChangedCallback(boost::bind(&LLPanelProfileLegacyPicks::onAccordionStateChanged, this, mClassifiedsAccTab));
	mClassifiedsAccTab->setDisplayChildren(false);
	
	LLUICtrl::CommitCallbackRegistry::ScopedRegistrar registar;
	registar.add("Pick.Info", boost::bind(&LLPanelProfileLegacyPicks::onClickInfo, this));
	registar.add("Pick.Edit", boost::bind(&LLPanelProfileLegacyPicks::onClickMenuEdit, this)); 
	registar.add("Pick.Teleport", boost::bind(&LLPanelProfileLegacyPicks::onClickTeleport, this));
	registar.add("Pick.Map", boost::bind(&LLPanelProfileLegacyPicks::onClickMap, this));
	registar.add("Pick.Delete", boost::bind(&LLPanelProfileLegacyPicks::onClickDelete, this));
	LLUICtrl::EnableCallbackRegistry::ScopedRegistrar enable_registar;
	enable_registar.add("Pick.Enable", boost::bind(&LLPanelProfileLegacyPicks::onEnableMenuItem, this, _2));
	...
	plus_registar.add("Picks.Plus.Action", boost::bind(&LLPanelProfileLegacyPicks::onPlusMenuItemClicked, this, _2));
	mEnableCallbackRegistrar.add("Picks.Plus.Enable", boost::bind(&LLPanelProfileLegacyPicks::isActionEnabled, this, _2));
...
void LLPanelProfileLegacyPicks::onPlusMenuItemClicked(const LLSD& param)
{
...
bool LLPanelProfileLegacyPicks::isActionEnabled(const LLSD& userdata) const
{
	return (!(userdata.asString() == LLStringExplicit("new_pick") && LLAgentPicksInfo::getInstance()->isPickLimitReached()));
}

bool LLPanelProfileLegacyPicks::isClassifiedPublished(LLClassifiedItem* c_item)
{
	if(c_item)
	{
...
void LLPanelProfileLegacyPicks::onAccordionStateChanged(const LLAccordionCtrlTab* acc_tab)
{
...
void LLPanelProfileLegacyPicks::onOpen(const LLSD& key)
{
	...
	LLPanelProfileLegacyTab::onOpen(key);
}

void LLPanelProfileLegacyPicks::onClosePanel()
{
...
void LLPanelProfileLegacyPicks::onListCommit(const LLFlatListView* f_list)
{
...
//static
void LLPanelProfileLegacyPicks::onClickDelete()
{
	...
	if (value.isDefined())
	{
		...
		LLNotificationsUtil::add("DeleteAvatarPick", args, LLSD(), boost::bind(&LLPanelProfileLegacyPicks::callbackDeletePick, this, _1, _2)); 
		return;
	}
	...
	if(value.isDefined())
	{
		...
		LLNotificationsUtil::add("DeleteClassified", args, LLSD(), boost::bind(&LLPanelProfileLegacyPicks::callbackDeleteClassified, this, _1, _2)); 
		return;
	}
}

bool LLPanelProfileLegacyPicks::callbackDeletePick(const LLSD& notification, const LLSD& response) 
{
...
bool LLPanelProfileLegacyPicks::callbackDeleteClassified(const LLSD& notification, const LLSD& response) 
{
	S32 option = LLNotificationsUtil::getSelectedOption(notification, response);
...
bool LLPanelProfileLegacyPicks::callbackTeleport( const LLSD& notification, const LLSD& response )
{
	S32 option = LLNotificationsUtil::getSelectedOption(notification, response);
...
//static
void LLPanelProfileLegacyPicks::onClickTeleport()
{
...
//static
void LLPanelProfileLegacyPicks::onClickMap()
{
	LLPickItem* pick_item = getSelectedPickItem();
	LLClassifiedItem* c_item = getSelectedClassifiedItem();
...
void LLPanelProfileLegacyPicks::onRightMouseUpItem(LLUICtrl* item, S32 x, S32 y, MASK mask)
{
	updateButtons();
...
void LLPanelProfileLegacyPicks::onDoubleClickPickItem(LLUICtrl* item)
{
	LLSD pick_value = mPicksList->getSelectedValue();
	...
	LLNotificationsUtil::add("TeleportToPick", args, LLSD(), boost::bind(&LLPanelProfileLegacyPicks::callbackTeleport, this, _1, _2)); 
}

void LLPanelProfileLegacyPicks::onDoubleClickClassifiedItem(LLUICtrl* item)
{
	LLSD value = mClassifiedsList->getSelectedValue();
	...
	LLNotificationsUtil::add("TeleportToClassified", args, LLSD(), boost::bind(&LLPanelProfileLegacyPicks::callbackTeleport, this, _1, _2)); 
}

void LLPanelProfileLegacyPicks::updateButtons()
{
...
void LLPanelProfileLegacyPicks::setProfilePanel(LLPanelProfileLegacy* profile_panel)
{
	mProfilePanel = profile_panel;
}


void LLPanelProfileLegacyPicks::buildPickPanel()
{
// 	if (mPickPanel == NULL)
// 	{
// 		mPickPanel = new LLPanelPick();
// 		mPickPanel->setExitCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelPickClose, this, NULL));
// 	}
}

void LLPanelProfileLegacyPicks::onClickPlusBtn()
{
...
void LLPanelProfileLegacyPicks::createNewPick()
{
	createPickEditPanel();
...
void LLPanelProfileLegacyPicks::createNewClassified()
{
...
void LLPanelProfileLegacyPicks::onClickInfo()
{
...
void LLPanelProfileLegacyPicks::openPickInfo()
{
...
void LLPanelProfileLegacyPicks::openClassifiedInfo()
{
...
void LLPanelProfileLegacyPicks::openClassifiedInfo(const LLSD &params)
{
	createClassifiedInfoPanel();
	getProfilePanel()->openPanel(mPanelClassifiedInfo, params);
}

void LLPanelProfileLegacyPicks::openClassifiedEdit(const LLSD& params)
{
...
void LLPanelProfileLegacyPicks::showAccordion(const std::string& name, bool show)
{
...
void LLPanelProfileLegacyPicks::onPanelPickClose(LLPanel* panel)
{
	getProfilePanel()->closePanel(panel);
}

void LLPanelProfileLegacyPicks::onPanelPickSave(LLPanel* panel)
{
	onPanelPickClose(panel);
	updateButtons();
}

void LLPanelProfileLegacyPicks::onPanelClassifiedSave(LLPanelClassifiedEdit* panel)
{
	if(!panel->canClose())
	{
		return;
...
void LLPanelProfileLegacyPicks::onPanelClassifiedClose(LLPanelClassifiedInfo* panel)
{
...
void LLPanelProfileLegacyPicks::createPickInfoPanel()
{
	if(!mPanelPickInfo)
	{
		mPanelPickInfo = LLPanelPickInfo::create();
		mPanelPickInfo->setExitCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelPickClose, this, mPanelPickInfo));
		mPanelPickInfo->setEditPickCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelPickEdit, this));
		mPanelPickInfo->setVisible(FALSE);
	}
}

void LLPanelProfileLegacyPicks::createClassifiedInfoPanel()
{
	mPanelClassifiedInfo = LLPanelClassifiedInfo::create();
	mPanelClassifiedInfo->setExitCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelClassifiedClose, this, mPanelClassifiedInfo));
	mPanelClassifiedInfo->setEditClassifiedCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelClassifiedEdit, this));
	mPanelClassifiedInfo->setVisible(FALSE);
}

void LLPanelProfileLegacyPicks::createClassifiedEditPanel(LLPanelClassifiedEdit** panel)
{
	if(panel)
	{
		LLPanelClassifiedEdit* new_panel = LLPanelClassifiedEdit::create();
		new_panel->setExitCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelClassifiedClose, this, new_panel));
		new_panel->setSaveCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelClassifiedSave, this, new_panel));
		new_panel->setCancelCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelClassifiedClose, this, new_panel));
		new_panel->setVisible(FALSE);
		*panel = new_panel;
	}
}

void LLPanelProfileLegacyPicks::createPickEditPanel()
{
	mPanelPickEdit = LLPanelPickEdit::create();
	mPanelPickEdit->setExitCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelPickClose, this, mPanelPickEdit));
	mPanelPickEdit->setSaveCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelPickSave, this, mPanelPickEdit));
	mPanelPickEdit->setCancelCallback(boost::bind(&LLPanelProfileLegacyPicks::onPanelPickClose, this, mPanelPickEdit));
	mPanelPickEdit->setVisible(FALSE);
}

void LLPanelProfileLegacyPicks::openPickEdit(const LLSD& params)
{
	createPickEditPanel();
	getProfilePanel()->openPanel(mPanelPickEdit, params);
}

void LLPanelProfileLegacyPicks::onPanelPickEdit()
{
	LLSD selected_value = mPicksList->getSelectedValue();
	if (selected_value.isUndefined()) return;
...
void LLPanelProfileLegacyPicks::onPanelClassifiedEdit()
{
	LLSD selected_value = mClassifiedsList->getSelectedValue();
...
LLClassifiedItem *LLPanelProfileLegacyPicks::findClassifiedById(const LLUUID& classified_id) const
{
...
void LLPanelProfileLegacyPicks::editClassified(const LLUUID&  classified_id)
{
...
void LLPanelProfileLegacyPicks::onClickMenuEdit()
{
...
bool LLPanelProfileLegacyPicks::onEnableMenuItem(const LLSD& user_data)
{
...
inline LLPanelProfileLegacy* LLPanelProfileLegacyPicks::getProfilePanel() const
{
